...
package org.tiqr.authenticator.qr.common;
...
/**
 * <p>
 * Sometimes the application wants to access advanced functionality exposed by Android APIs that are only available in later versions of the platform. While
 * {@link Build.VERSION} can be used to determine the device's API level and alter behavior accordingly, and it is possible to write code that uses both old and
 * new APIs selectively, such code would fail to load on older devices that do not have the new API methods.
 * </p>
 * 
 * <p>
 * It is necessary to only load classes that use newer APIs than the device may support after the app has checked the API level. This requires reflection,
 * loading one of several implementations based on the API level.
 * </p>
 * 
 * <p>
 * This class manages that process. Subclasses of this class manage access to implementations of a given interface in an API-level-aware way. Subclasses
 * implementation classes <em>by name</em>, and the minimum API level that the implementation is compatible with. They also provide a default implementation.
 * </p>
 * 
 * <p>
